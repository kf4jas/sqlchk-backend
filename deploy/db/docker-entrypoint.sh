...
add_ssl_to_postgresql() {
    /usr/bin/openssl req -new -newkey rsa:4096 -nodes -keyout /var/lib/postgresql/data/server.key -out /var/lib/postgresql/data/server.csr -subj "${SSL_SUBJ_STR}"
    /usr/bin/openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -keyout /var/lib/postgresql/data/server.key -out /var/lib/postgresql/data/server.crt -subj "${SSL_SUBJ_STR}"
